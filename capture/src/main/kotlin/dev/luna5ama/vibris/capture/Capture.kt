...
import dev.luna5ama.vibris.common.*
...
internal class CaptureContext {
    ...
    fun getImageIndex(imageID: Int): Int = MemoryStack {
        ...
        if (VIBRIS_DEBUG) println("Capturing image ID: $imageID")
        ...
        shaderInfo.uniforms.values.asSequence()
            ...
            .forEach {
                fun getData(elementInfo: StructAllocator.ElementInfo) {
                    if (VIBRIS_DEBUG) println("Capturing default uniform: ${it.name}")
                    if (VIBRIS_DEBUG) println("offset=${elementInfo.offset}, size=${elementInfo.size}, totalSize=${this@struct.size}")
...
fun beginGlCapture(outputPath: Path) {
    check(activeCaptureContext == null) { "A vibris capture is already active" }
    glFinish()
...
fun endGlCapture(): Thread {
    val captureContext = checkNotNull(activeCaptureContext) { "No active vibris capture" }
    val outputPath = checkNotNull(activeCaptureOutputPath) { "No active vibris capture output path" }
    glFinish()
