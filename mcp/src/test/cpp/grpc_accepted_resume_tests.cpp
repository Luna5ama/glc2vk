...
void accepted_request_resumes_after_disconnect() {
    ...
    request.mutable_submit_job()->set_request_id("job-resume");
    ...
    // Then: the event stream is surfaced and reconnect uses ResumeRequest without duplicating SubmitJob.
    ...
    const std::vector expected{
        ...
        proto::ServerMessage::kResumeState,
    ...
    require(server.submit_jobs() == 1 && server.resume_requests() == 1 && server.duplicate_submits() == 0,
        "Reconnect duplicated SubmitJob instead of sending one ResumeRequest.");
}
