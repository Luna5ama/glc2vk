...
layout(std430, binding = 0) restrict readonly buffer GlobalData {
...
layout(std430, binding = 1) restrict readonly buffer IndirectComputeData { uint indirectComputeData[]; };
...
layout(rgba32ui) uniform writeonly uimage2D colorimg8;
layout(rgba8) uniform writeonly image2D colorimg9;
layout(r32f) uniform writeonly image2D colorimg10;
layout(rgba16f) uniform writeonly image2D colorimg11;
void main() {
	ivec2 texelPos = ivec2(gl_GlobalInvocationID.xy);
	if (all(lessThan(texelPos, ivec2(uval_mainImageSizeI)))) {
		imageStore(colorimg8, texelPos + ivec2(0), uvec4(0u));
		imageStore(colorimg9, texelPos + ivec2(0), vec4(0.0f));
		imageStore(colorimg10, texelPos + ivec2(0), vec4(-65536.0f));
		imageStore(colorimg11, texelPos + ivec2(0), vec4(1.0f, 1.0f, 1.0f, 0.0f));
	}
}
